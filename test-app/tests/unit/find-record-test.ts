...
import {
  IdRequiredError,
  IdTypeError,
} from 'ember-data-resources/-private/resources/errors';
...
module('findRecord', function (hooks) {
  ...
  module('in js', function (hooks) {
    ...
    test('it works when callback returns id', async function (assert) {
      ...
      instance.id = '2';
      assert.true(
        instance.blog.hasRan,
        'hasRan 2 (true, because we only changed the id)'
      );
      await settled();
    ...
    test('it should return an IdRequiredError when id is undefined', async function (assert) {
      ...
      assert.ok(
        instance.blog.error?.message.includes('blog'),
        'error message has modelName'
      );
